...
public:
    ...
    uint32_t getNodeCount() const {
        return (!mapping ? nodeIndex.size() : idToPos->size());
    }
        ...
        while (firstTime || pos == NONE || !nodeIndex[pos].edges) { // Ignore nodes with no edges
            if (pos == NONE && nodeIndex.size() || pos != NONE && pos < nodeIndex.size() - 1) {
                ...
                for ( ; offset < nextNodeOffset ; offset++) {
                    if (idToPos->find(edgeBuffer[offset]) == idToPos->end()) { // For subgraphs generated by buildNDegreeSubgraph
                        continue;
                    }
    ...
    void getNextEdge(GraphTraversal &graphTraversal) const {
        ...
        for ( ; graphTraversal.curEdgeOffset  < nextNodeOffset ; graphTraversal.curEdgeOffset++) {
            if (idToPos->find(edgeBuffer[graphTraversal.curEdgeOffset]) == idToPos->end()) { // For subgraphs generated by buildNDegreeSubgraph
                continue;
            }
    ...
    void goToNode(const uint32_t &node, GraphTraversal &graphTraversal) const {
        ...
        graphTraversal.curEdgeOffset = (nodeIndex[pos].edges ? nodeIndex[pos].offset : NONE);
        //assert(graphTraversal.curEdgeOffset == NONE || !nodeIndex[(!mapping ? graphTraversal.curEdgeOffset : idToPos->at(graphTraversal.curEdgeOffset])).removed);
    }
